...
def detect_red_ball(imageFrame):
    ...
    while True:
        ...
        if len(contours) > 0:
            ...
            if M['m00'] != 0:
                ...
                print("Zero detected, restarting zeroloop")
                line_following()
        else:
            print("No line detected, stopping")
            stop()

        # Check for yellow color to switch to red ball detection
        lower_yellow = np.array([20, 100, 100])
        upper_yellow = np.array([30, 255, 255])
        yellow_mask = cv2.inRange(hsv_frame, lower_yellow, upper_yellow)
        if np.sum(yellow_mask) > 0:
            print("Yellow background detected, switching to red ball detection")
...
def main():
    ...
    while True:
        # Line following until a yellow background is detected
        line_following()
        ...
        while True:
            ...
            frame, hsv_frame = detect_red_ball(frame)

            # Displaying the output
            cv2.imshow("Red Ball Detection in Real-Time", frame)
            cv2.imshow("Red Ball Detection in Real-Time HSV", hsv_frame)

            # Exiting the loop if 'q' is pressed
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break

        # Break the main loop if 'q' is pressed
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
